  ...
  return (
    <footer id="kontak" className="bg-[#0A1627] text-white">
      <div className="site-container grid gap-10 py-14 md:grid-cols-2 lg:grid-cols-[1.35fr_.7fr_.7fr_1fr] lg:gap-12 lg:py-16">
        <div>
          <BrandMark inverse />
          <p className="mt-5 max-w-[370px] text-sm leading-7 text-slate-400">Layanan sewa mobil dan motor terpercaya di Bandung. Armada terawat, harga transparan, dan pelayanan profesional untuk setiap perjalanan Anda.</p>
        ...
        <div>
          ...
          <div className="mt-5 space-y-3.5 text-[13px] leading-6 text-slate-400">
            <p className="flex items-start gap-3"><MapPin className="mt-0.5 size-4 shrink-0 text-[#60A5FA]" />Bandung, Jawa Barat — melayani seluruh area Bandung Raya.</p>
            ...
            <a href="mailto:info@drivemate.co.id" className="flex items-center gap-3 hover:text-[#60A5FA]"><Mail className="size-4 text-[#60A5FA]" />info@drivemate.co.id</a>
          </div>
        </div>
      </div>
      <div className="site-container flex flex-col gap-3 border-t border-white/10 py-5 text-center text-xs text-slate-500 sm:flex-row sm:justify-between sm:text-left"><span>© 2026 DriveMate. All rights reserved.</span><span>Made with trust in Bandung • Your Trusted Travel Companion.</span></div>
      <a aria-label="Hubungi DriveMate via WhatsApp" href={bookingUrl()} target="_blank" rel="noreferrer" className="whatsapp-attention fixed bottom-5 right-5 z-[80] grid size-14 place-items-center rounded-full bg-[#25D366] text-white shadow-[0_12px_30px_rgba(22,163,74,.3)] ring-4 ring-white/80 transition hover:scale-110 hover:bg-[#20BD5A] hover:[animation-play-state:paused]"><WhatsappLogoIcon className="relative z-10 size-8" weight="fill" /></a>
    </footer>
  );
}
