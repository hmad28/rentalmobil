    ...
    <section className="bg-[#F8FAFC] pb-10 pt-5 lg:pb-20 lg:pt-7">
      <div className="site-container grid overflow-hidden rounded-2xl border border-white/10 bg-[linear-gradient(110deg,#0B1728_0%,#111827_55%,#10274A_100%)] shadow-[0_20px_50px_rgba(15,23,42,.16)] sm:grid-cols-2 lg:grid-cols-4">
        {stats.map(({ value, label, icon: Icon }, index) => (
          <div key={label} className={`group flex items-center gap-4 px-8 py-7 transition-colors duration-300 hover:bg-white/[0.045] ${index > 0 ? "border-t border-white/15 sm:border-t-0 sm:border-l" : ""} ${index === 2 ? "sm:border-l-0 sm:border-t lg:border-l lg:border-t-0" : ""}`}>
            <span className="grid size-12 place-items-center rounded-2xl border border-[#60A5FA]/20 bg-[#2563EB]/15 text-[#60A5FA] transition duration-300 group-hover:-translate-y-1 group-hover:scale-105 group-hover:bg-[#2563EB]/25"><Icon className="size-6" strokeWidth={1.7} /></span>
